...
If you run `npm test`, you'll get a failed build from a number of unused imports. Let's fix those by adding the first test of `getAll()`. You can see
how the `MockBackend` is used to mock results and set the response.
...
Running `npm test` should result in "10 tests completed". Add couple more tests for filtering by search term and fetching by id.
...
`npm run serve.coverage`. You'll notice that our new components and service could use some additional coverage. I'll leave that as a task
for the reader. Please send a pull request to https://github.com/mraible/angular2-tutorial[this project] if you're ambitious.
