...
by creating `people.json` to hold our data.
...
In `SearchComponent`, add imports for these classes.
...
and I highly recommend it if you're learning Angular 2. I also found Chariot Solution's article on
http://chariotsolutions.com/blog/post/angular2-observables-http-separating-services-components/[Angular2 Observables, Http, and separating services and components] to be quite helpful.

Also, kudos to all the pioneers in Angular 2 land that've been using it and writing about it on blogs and Stack Overflow.
